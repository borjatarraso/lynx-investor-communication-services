...
from lynx_comm.core.storage import _sanitize_ticker, set_mode, get_company_dir
from lynx_comm.core.ticker import is_isin
from lynx_comm.metrics.calculator import (
    calc_valuation, calc_solvency, calc_growth, calc_share_structure,
    calc_comm_quality, calc_intrinsic_value,
)
from lynx_comm.export import export_report, ExportFormat
...
class TestNanInfHandling:
    ...
    def test_valuation_with_nan(self):
        v = calc_valuation(
            ...
            [], CompanyTier.NANO, CompanyStage.STARTUP,
        )
    ...
    def test_solvency_with_nan(self):
        ...
        s = calc_solvency({"marketCap": 100}, stmts, CompanyTier.NANO, CompanyStage.STARTUP)
...
class TestZeroDivision:
    def test_zero_shares(self):
        ...
        g = calc_growth(stmts, CompanyTier.NANO, CompanyStage.STARTUP)
        assert g is not None

    def test_zero_market_cap(self):
        v = calc_valuation({"marketCap": 0, "totalCash": 100}, [], CompanyTier.NANO, CompanyStage.STARTUP)
...
class TestCalcEdgeCases:
    ...
    def test_share_structure_no_info(self):
        ss = calc_share_structure({}, [], GrowthMetrics(), CompanyTier.NANO, CompanyStage.STARTUP)
