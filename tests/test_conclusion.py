...
from lynx_comm.models import (
    ...
    CommQualityIndicators, ShareStructure,
)
...
def explorer_report():
    ...
    r.comm_quality = CommQualityIndicators(quality_score=65.0,
...
class TestGenerateConclusion:
    ...
    def test_growth_has_stage_note(self, explorer_report):
        c = generate_conclusion(explorer_report)
        assert c.stage_note != ""
        assert ("Growth" in c.stage_note or "ARPU" in c.stage_note
                or "Subscriber" in c.stage_note or "engagement" in c.stage_note.lower())
    ...
    def test_category_scores_present(self, explorer_report):
        ...
        assert "comm_quality" in c.category_scores
    ...
    def test_screening_checklist_present(self, explorer_report):
        ...
        assert "insider_alignment" in c.screening_checklist
        assert "rule_of_40_pass" in c.screening_checklist
        # Comm-services-specific checks
        assert "leverage_discipline" in c.screening_checklist
        assert "capex_discipline" in c.screening_checklist
