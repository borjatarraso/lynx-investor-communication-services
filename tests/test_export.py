...
@pytest.fixture
def sample_report():
    p = CompanyProfile(ticker="TEST", name="Test Streaming Corp",
    ...
    r = AnalysisReport(
        ...
        valuation=ValuationMetrics(pb_ratio=5.0, cash_to_market_cap=0.15, ev_revenue=6.0,
                                   ev_gross_profit=9.0, ev_to_subscriber=400.0,
                                   fcf_yield=0.04),
        solvency=SolvencyMetrics(cash_runway_years=3.0, total_cash=200_000_000,
                                 current_ratio=4.0, debt_to_equity=0.1,
                                 net_debt_to_ebitda=2.5, capex_to_revenue=0.08),
        growth=GrowthMetrics(shares_growth_yoy=0.03, revenue_growth_yoy=0.35,
                             rd_intensity=0.10, content_intensity=0.40,
                             subscriber_growth_yoy=0.20, dau_mau_ratio=0.65),
        comm_quality=CommQualityIndicators(quality_score=72.0,
                                           competitive_position="Strong — High-Quality Operator / Platform"),
