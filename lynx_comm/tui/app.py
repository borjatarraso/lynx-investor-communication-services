"""Textual UI for Lynx Communication Services Analysis."""
...
class ExplainModal(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        from lynx_comm.metrics.explanations import (
...
class MetricListModal(ModalScreen[str]):
    ...
    def compose(self) -> ComposeResult:
        from lynx_comm.metrics.explanations import list_metrics
...
class SearchModal(ModalScreen[str]):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="search-dialog"):
            ...
            yield Input(
                placeholder="e.g. OCO.V, DML.TO, UUUU, FUU.V",
                id="search-input",
            )
            yield Label(
...
class ReportView(VerticalScroll):
    ...
    def add_stage(self, stage: str, report: AnalysisReport) -> None:
        ...
        if stage == "profile":
            ...
            self._mount_section("Sector Quality", _build_comm_quality(report))
...
class LynxCommApp(PagingAppMixin, App):
    TITLE = "Lynx Communication Services Analysis"
    SUB_TITLE = "Telecom, Cable, Streaming, Internet, Social, Advertising, Publishing & Gaming"
    CSS = """
    #search-dialog {
        width: 60;
        height: auto;
    ...
    def on_mount(self) -> None:
        from lynx_comm.tui.themes import register_all_themes, THEME_NAMES
        try:
            register_all_themes(self)
            self.theme = THEME_NAMES[0]  # comm-dark
        except Exception:
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Static(
            "[bold blue]  lynx-comm  [/]\n\n"
            "[bold]Communication Services Analysis Tool[/]\n"
        ...
        if isinstance(focused, DataTable):
            ...
            if section == "conclusion":
                ...
                try:
                    ...
                    for cat in ("valuation", "profitability", "solvency", "growth", "comm_quality"):
    ...
    def action_cycle_theme(self) -> None:
        from lynx_comm.tui.themes import THEME_NAMES
    ...
    def action__ee_lynx(self) -> None:
        from lynx_comm.easter import LYNX_ASCII, WOLF_ASCII, BULL_ASCII
        import random
        ...
        try:
            ...
            if isinstance(e, SectorMismatchError):
                self.call_from_thread(
                    ...
                    f"[bold red]{e}[/]\n\n"
                    f"[bold blink red]This tool is specialized ONLY for Communication Services:[/]\n"
                    f"[red]Telecom | Cable | Streaming | Internet | Social | Advertising | Publishing | Gaming | Broadcasting[/]\n\n"
...
def _build_profile_table(r: AnalysisReport) -> DataTable:
    ...
    _r2(t, "Sub-Sector", _s(p.comm_category.value if hasattr(p.comm_category, "value") else p.comm_category))
...
def _build_valuation(r: AnalysisReport) -> DataTable:
    ...
    _rm_rel(t, "EV/Gross Profit", _num(v.ev_gross_profit), _thr(v.ev_gross_profit, [(5, "Cheap"), (10, "Fair"), (20, "Expensive")], "Very Expensive"), "ev_gross_profit", rel("ev_gross_profit"))
    _rm_rel(t, "EV/ARR", _num(v.ev_to_arr), _thr(v.ev_to_arr, [(5, "Cheap"), (10, "Fair"), (20, "Expensive")], "Very Expensive"), "ev_to_arr", rel("ev_to_arr"))
    _rm_rel(t, "EV/Employee", _money(v.ev_per_employee), "", "ev_per_employee", rel("ev_per_employee"))
    _rm_rel(t, "Rule of 40 Adj Multiple", _num(v.rule_of_40_adj_multiple), "", "rule_of_40_adj_multiple", rel("rule_of_40_adj_multiple"))
...
def _build_profitability(r: AnalysisReport) -> DataTable | Static:
    ...
    _rm_rel(t, "Rule of 40", _pct(p.rule_of_40), _thr(p.rule_of_40, [(0.20, "Weak"), (0.40, "Healthy"), (0.60, "Strong")], "Exceptional"), "rule_of_40", rel("rule_of_40"))
    _rm_rel(t, "Rule of 40 (EBITDA)", _pct(p.rule_of_40_ebitda), "", "rule_of_40_ebitda", rel("rule_of_40_ebitda"))
    _rm_rel(t, "Magic Number", _num(p.magic_number), _thr(p.magic_number, [(0.5, "Weak"), (0.75, "Decent"), (1.0, "Good")], "Excellent"), "magic_number", rel("magic_number"))
    _rm_rel(t, "SBC / Revenue", _pct(p.sbc_to_revenue), _thr(p.sbc_to_revenue, [(0.05, "Low"), (0.15, "Moderate"), (0.25, "High")], "Very High"), "sbc_to_revenue", rel("sbc_to_revenue"))
    _rm_rel(t, "SBC / FCF", _pct(p.sbc_to_fcf), "", "sbc_to_fcf", rel("sbc_to_fcf"))
    _rm_rel(t, "GAAP vs Adj Gap", _pct(p.gaap_vs_adj_gap), "", "gaap_vs_adj_gap", rel("gaap_vs_adj_gap"))
    return t
...
def _build_solvency(r: AnalysisReport) -> DataTable:
    ...
    _rm_rel(t, "Cash Coverage (months)", f"{s.cash_coverage_months:.1f}" if s.cash_coverage_months is not None else "N/A", "", "cash_coverage_months", rel("cash_coverage_months"))
    _rm_rel(t, "Capex / Revenue", _pct(s.capex_to_revenue), _thr(s.capex_to_revenue, [(0.02, "Light"), (0.05, "Moderate"), (0.10, "Heavy")], "Very Heavy"), "capex_to_revenue", rel("capex_to_revenue"))
    _rm_rel(t, "RPO Coverage", _num(s.rpo_coverage), "", "rpo_coverage", rel("rpo_coverage"))
    _rm_rel(t, "Deferred Revenue Ratio", _pct(s.deferred_revenue_ratio), "", "deferred_revenue_ratio", rel("deferred_revenue_ratio"))
    _rm_rel(t, "Goodwill / Assets", _pct(s.goodwill_to_assets), _thr(s.goodwill_to_assets, [(0.10, "Low"), (0.30, "Moderate"), (0.50, "Heavy")], "Very Heavy"), "goodwill_to_assets", rel("goodwill_to_assets"))
    return t
...
def _build_growth(r: AnalysisReport) -> DataTable:
    ...
    _rm_rel(t, "ARR Growth (YoY)", _pct(g.arr_growth_yoy), _growth_assess(g.arr_growth_yoy), "arr_growth_yoy", rel("arr_growth_yoy"))
    _rm_rel(t, "Net Revenue Retention", _pct(g.net_revenue_retention), _thr(g.net_revenue_retention, [(1.0, "Churning"), (1.10, "Healthy"), (1.20, "Strong")], "Best-in-class"), "net_revenue_retention", rel("net_revenue_retention"))
    _rm_rel(t, "Gross Revenue Retention", _pct(g.gross_revenue_retention), "", "gross_revenue_retention", rel("gross_revenue_retention"))
    _rm_rel(t, "R&D Intensity", _pct(g.rd_intensity), _thr(g.rd_intensity, [(0.05, "Low"), (0.15, "Moderate"), (0.25, "Heavy")], "Very Heavy"), "rd_intensity", rel("rd_intensity"))
    _rm_rel(t, "R&D Growth (YoY)", _pct(g.rd_growth_yoy), "", "rd_growth_yoy", rel("rd_growth_yoy"))
    _rm_rel(t, "S&M Intensity", _pct(g.sales_marketing_intensity), "", "sales_marketing_intensity", rel("sales_marketing_intensity"))
    _rm_rel(t, "Employee Growth (YoY)", _pct(g.employee_growth_yoy), "", "employee_growth_yoy", rel("employee_growth_yoy"))
    _rm_rel(t, "Revenue / Employee", _money(g.revenue_per_employee), "", "revenue_per_employee", rel("revenue_per_employee"))
    return t
...
def _build_share_structure(r: AnalysisReport) -> Static:
    ...
    _row("RSU Outstanding", f"{ss.rsu_outstanding:,.0f}" if ss.rsu_outstanding else "N/A", rel("rsu_outstanding"))
    ...
    _row("Dual-Class Structure", ("Yes" if ss.dual_class_structure else ("No" if ss.dual_class_structure is False else "N/A")), rel("dual_class_structure"))
    _row("SBC Overhang Risk", _s(ss.sbc_overhang_risk) if ss.sbc_overhang_risk else "N/A", rel("sbc_overhang_risk"))
    _row("Assessment", _s(ss.share_structure_assessment), rel("share_structure_assessment"))
    return _rich_table_widget(t)


def _build_comm_quality(r: AnalysisReport) -> Static:
    """Build a Rich Table for tech quality with multi-line cell wrapping."""
    m = r.comm_quality
    stage = _get_stage(r)
    tier = _get_tier(r)
    t = RichTable(show_lines=True, border_style="yellow", expand=True)
    ...
    def rel(key: str) -> Relevance:
        return _get_rel(key, tier, "comm_quality", stage)
    ...
    _row("Moat", _s(m.moat_assessment), rel("moat_assessment"))
    _row("Moat Type", _s(m.moat_type), rel("moat_type"))
    _row("Platform Position", _s(m.platform_position), rel("platform_position"))
    _row("Unit Economics", _s(m.unit_economics), rel("unit_economics"))
    _row("Rule of 40", _s(m.rule_of_40_assessment), rel("rule_of_40_assessment"))
    _row("R&D Efficiency", _s(m.rd_efficiency_assessment), rel("rd_efficiency_assessment"))
    _row("SBC Risk", _s(m.sbc_risk_assessment), rel("sbc_risk_assessment"))
    _row("Founder-Led", _s(m.founder_led), rel("founder_led"))
    ...
    _row("Revenue Predictability", _s(m.revenue_predictability), rel("revenue_predictability"))
    ...
    _row("Catalyst Density", _s(m.catalyst_density), rel("catalyst_density"))
...
def _build_iv(r: AnalysisReport) -> Static:
    ...
    if iv.ev_sales_implied_price is not None:
        t.add_row(f"{tag('EV/Sales')}EV/Sales Implied", f"${iv.ev_sales_implied_price:.4f}", _mos(iv.margin_of_safety_ev_sales))
...
def _build_market_intelligence(r: AnalysisReport) -> Static:
    ...
    if mi.benchmark_name or mi.sector_etf_name:
        t = RichTable(show_lines=True, border_style="cyan", expand=True, title="Benchmark & Sector Context")
        ...
        if mi.benchmark_name and mi.benchmark_price:
            t.add_row("Benchmark", mi.benchmark_name, f"${mi.benchmark_price:,.2f}")
