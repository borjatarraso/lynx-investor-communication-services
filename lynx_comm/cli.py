"""Command-line interface for lynx-comm."""
...
from lynx_comm import __author__, __author_email__, __license__, __version__, __year__, SUITE_LABEL
...
def build_parser():
    parser = argparse.ArgumentParser(
        prog="lynx-comm",
        description="Lynx Communication Services Analysis — Telecom, Wireless, Cable, Streaming, Internet, Social, Advertising & Gaming fundamental analysis tool.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  lynx-comm -p GOOGL                           Internet content platform\n"
            "  lynx-comm -p TMUS                            Wireless carrier\n"
            "  lynx-comm -p NFLX                            Streaming entertainment\n"
            "  lynx-comm -p META --refresh                  Force fresh data\n"
            '  lynx-comm -p "Disney"                        Search by name\n'
            "  lynx-comm -p -i                              Interactive mode\n"
            "  lynx-comm -t VZ                              Testing mode\n"
        ),
    )
...
def run_cli():
    ...
    if "--b2m" in sys.argv:
        ...
        from lynx_comm.easter import rich_rocket, rich_fortune
        c = Console()
        rich_rocket(c)
        rich_fortune(c)
        return
    ...
    if "--about" in sys.argv:
        from rich.console import Console; from rich.panel import Panel; from lynx_comm import get_about_text
        about = get_about_text(); c = Console(stderr=True)
        logo = about.get('logo_ascii', '')
        logo_block = f"[bold green]{logo}[/]\n" if logo else ""
        c.print(Panel(f"{logo_block}[bold blue]{about['name']} v{about['version']}[/]\n[dim]Part of {about['suite']} v{about['suite_version']}[/]\n\n"
    ...
    if "--explain" in sys.argv:
        from rich.console import Console; from rich.panel import Panel; from rich.table import Table
        from lynx_comm.metrics.explanations import get_explanation, list_metrics
    ...
    from lynx_comm.core.storage import set_mode, is_testing
    ...
    if args.interactive:
        from lynx_comm.interactive import run_interactive; run_interactive(); return
    if args.tui:
        from lynx_comm.tui.app import run_tui; run_tui(); return
    if args.gui:
        from lynx_comm.gui.app import run_gui; run_gui(args); return
    if args.search:
    ...
    from lynx_comm.display import display_report_stage
    try:
        ...
        if args.export:
            from pathlib import Path; from lynx_comm.export import ExportFormat, export_report
...
def _cmd_list_cache(con):
    from rich.table import Table; from lynx_comm.core.storage import list_cached_tickers, get_mode
