"""Rich console display for Communication Services analysis reports — stage and tier aware.
...
def _a_pe(val):
    ...
    if val < 12:
        return "[green]Very cheap — possible value or distressed comm-services name[/]"
    if val < 18:
...
def _a_ps(val):
    ...
    if val < 2.0:
        return "[green]Cheap — typical of mature cash-generative carrier or platform[/]"
    if val < 4.0:
...
def _a_divy(val):
    if _isna(val):
        return "[dim]No dividend — typical for growth-stage comm services[/]"
    if val == 0:
...
def _a_ctm(val):
    ...
    if val > 0.05:
        return "[#ff8800]Low cash backing — financing risk for early-stage comm-services name[/]"
...
def _a_roa(val):
    ...
    if val < 0.08:
        return "[green]Strong ROA for asset-heavy carrier (telecom/cable)[/]"
...
def _a_dil(val):
    ...
    if val < 0.05:
        return "[yellow]Modest dilution — typical of funded growth-stage comm-services[/]"
    if val < 0.10:
...
def display_full_report(report):
    ...
    if report.comm_quality:
        _display_comm_quality(report)
...
def display_report_stage(stage, report):
    ...
    handlers = {
        ...
        "comm_quality": lambda: (
            _progressive_stages_seen.add("comm_quality"),
            _display_comm_quality(report),
        ),
...
def _display_header(report):
    ...
    console.print(Panel(
        header,
        title="[bold]LYNX Communication Services Analysis[/]",
        border_style="blue",
    ))
    ...
    console.print(Panel(
        ...
        f"[cyan]Sub-Sector:[/] {p.comm_category.value}  |  "
...
def _display_solvency(report):
    ...
    if s.capex_to_revenue is not None:
        ...
        msg = ("Asset-light (internet platform / agency)" if ctr < 0.03
...
def _display_comm_quality(report):
    """Sector quality assessment — uses relevance markers via get_relevance with category='comm_quality'."""
    m = report.comm_quality
    if m is None:
        return
    tier, stage = report.profile.tier, report.profile.stage
    rel = lambda key: get_relevance(key, tier, "comm_quality", stage)

    t = Table(title="Sector Quality Assessment", show_lines=True, border_style="bold yellow")
...
def _add_quality_row(table, label, value, relevance):
    """Add a row to the sector quality table with relevance-based styling."""
    if relevance == Relevance.IRRELEVANT:
        return
    style, prefix = _STYLE[relevance]
    sl = f"{prefix}[{style}]{label}[/]" if style else f"{prefix}{label}"
...
def _display_intrinsic_value(report):
    ...
    # EV/Sales implied price (peer-multiple intrinsic comp)
    if getattr(iv, "ev_sales_implied_price", None):
        t.add_row(
            _m("EV/Sales") + "EV/Sales Implied",
            f"${iv.ev_sales_implied_price:.2f}",
            _mos_color(getattr(iv, "margin_of_safety_ev_sales", None)),
        )
    ...
    if iv.current_price and any([
        ...
        getattr(iv, "margin_of_safety_ev_sales", None),
    ]):
        mos_values = [
            v for v in [
                iv.margin_of_safety_dcf, iv.margin_of_safety_graham,
...
def _display_screening(c):
    ...
    t = Table(title="Comm Services Screening Checklist", border_style="cyan")
...
def _display_market_intelligence(report):
    ...
    # --- Comm-Services Benchmark & Sector Context ---
    if mi.benchmark_name or mi.sector_etf_name:
        t = Table(title="Comm-Services Benchmark & Sector Context", show_lines=True, border_style="cyan")
        t.add_column("Indicator", style="bold", min_width=22, no_wrap=True)
    ...
    # --- Communication Services Investment Disclaimers ---
    if mi.disclaimers:
        console.print(Panel(
