"""Main analysis orchestrator for Communication Services companies."""
...
from lynx_comm.core.fetcher import fetch_company_profile, fetch_financial_statements, fetch_info
from lynx_comm.core.news import fetch_all_news
from lynx_comm.core.reports import download_top_filings, fetch_sec_filings
from lynx_comm.core.storage import get_cache_age_hours, has_cache, load_cached_report, save_analysis_report
from lynx_comm.core.ticker import resolve_identifier
from lynx_comm.metrics.calculator import (
    calc_efficiency, calc_growth, calc_intrinsic_value, calc_market_intelligence,
    calc_comm_quality, calc_profitability, calc_share_structure, calc_solvency,
    calc_valuation,
)
from lynx_comm.models import (
    ...
    CommQualityIndicators, NewsArticle,
...
_ALLOWED_SECTORS = {"communication services", "communications"}
_ALLOWED_INDUSTRIES = {
    "telecom services", "telecommunications services", "wireless communications",
    "wireless carrier", "diversified telecommunication services",
    "integrated telecommunication services", "alternative carriers",
    "cable", "cable & satellite", "media",
    "entertainment", "movies & entertainment", "interactive home entertainment",
    "interactive media & services", "internet content & information",
    "publishing", "broadcasting", "broadcasting (entertainment)",
...
class SectorMismatchError(Exception):
    """Raised when a company does not belong to Communication Services."""
    pass


def _validate_sector(profile: CompanyProfile) -> None:
    """Check if the company belongs to Communication Services.
    ...
    # Communication-Services-specific terms (word-boundary matched) used to
    # detect comm-services activity when sector classification is mislabeled.
    comm_specific = [
        r"\bwireless carrier\b", r"\bmobile network operator\b", r"\bmno\b",
        r"\b5g\b.*\bnetwork\b", r"\blte\b.*\bnetwork\b",
        r"\bspectrum (license|holdings|auction)\b",
        r"\bfiber-to-the-home\b", r"\bftth\b", r"\bfiber broadband\b",
        r"\bcable operator\b", r"\bcable television\b", r"\bsatellite tv\b",
        r"\bstreaming service\b", r"\bsubscription video\b", r"\bsvod\b",
        r"\bvideo on demand\b", r"\bover-the-top\b", r"\bott video\b",
        r"\bsearch engine\b", r"\binternet content\b", r"\binternet platform\b",
        r"\bsocial (media|network)\b", r"\bmessaging platform\b",
        r"\bdigital advertising\b", r"\bad tech\b", r"\bad exchange\b",
        r"\badvertising agency\b", r"\bmedia agency\b",
        r"\bnewspaper\b", r"\bmagazine\b", r"\bnews media\b",
        r"\bbroadcast television\b", r"\bbroadcasting\b", r"\bradio station\b",
        r"\bvideo game (publisher|developer)\b", r"\belectronic gaming\b",
        r"\bmobile gaming\b", r"\bconsole gaming\b", r"\bgame studio\b",
        r"\binteractive entertainment\b",
    ]
    if any(re.search(pattern, desc) for pattern in comm_specific):
        return
    ...
    message = (
        ...
        f"Lynx Communication Services Analysis is specialized exclusively for:\n"
        f"  - Telecom Services (wireless, wireline, fiber)\n"
        f"  - Cable & Satellite operators\n"
        f"  - Streaming, Entertainment & Diversified Media\n"
        f"  - Internet Content & Search\n"
        f"  - Social Media & Interactive Media\n"
        f"  - Digital Advertising & Advertising Agencies\n"
        f"  - Publishing, Broadcasting (TV/Radio)\n"
        f"  - Electronic Gaming & Multimedia\n\n"
        f"For other sectors, use the matching Lynx specialist "
        f"(Information Technology, Basic Materials, Energy, Healthcare, "
        f"Financials, Industrials, Utilities, Real Estate, "
        f"Consumer Discretionary, Consumer Staples)."
    )
    message += format_agent_suggestion(
        profile, current_agent="lynx-investor-communication-services",
    )
...
def run_progressive_analysis(
    ...
    profile.stage = stage
    profile.comm_category = classify_category(profile.description, profile.industry)
    ...
    if profile.comm_category.value != "Other Communication Services":
        console.print(f"[cyan]Sub-Sector:[/] {profile.comm_category.value}")
...
def _dict_to_report(d: dict) -> AnalysisReport:
    ...
    return AnalysisReport(
        ...
        comm_quality=_maybe(CommQualityIndicators, "comm_quality"),
