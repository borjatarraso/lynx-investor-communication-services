"""Communication-Services-focused report synthesis engine.

Generates the weighted overall score + verdict + strengths/risks for a
Communication Services company. Weights adapt by both tier and Communication
Services lifecycle stage. Screening checklist covers the 10 most important
comm-services quality criteria (ARPU/Churn/CPGA, leverage, FCF discipline,
content economics, dilution).
"""
...
from lynx_comm.models import AnalysisConclusion, AnalysisReport, CompanyStage, CompanyTier, JurisdictionTier
...
# Weights: (valuation, profitability, solvency, growth, comm_quality)
_WEIGHTS = {
...
def generate_conclusion(report: AnalysisReport) -> AnalysisConclusion:
    ...
    quality_score = _safe(report.comm_quality.quality_score) if report.comm_quality else 0
    ...
    c.category_scores = {"valuation": round(val_score, 1), "profitability": round(prof_score, 1),
                         ...
                         "comm_quality": round(quality_score, 1)}
...
def _score_solvency(r: AnalysisReport) -> float:
    ...
    # Net Debt / EBITDA — the headline leverage metric for telecom/cable.
    # Carriers/cable run leveraged by design; thresholds adjusted accordingly.
    nde = _safe(s.net_debt_to_ebitda, None)
    if nde is not None:
        if nde < 1.5: score += 12
        elif nde < 3.0: score += 6
        elif nde < 4.0: score -= 0
        elif nde < 5.0: score -= 8
        else: score -= 15

    # Capex intensity — flag the build phase but don't penalize harvest phase
    capex_rev = _safe(s.capex_to_revenue, None)
    if capex_rev is not None:
        if capex_rev > 0.30:
            score -= 6  # likely peak capex (5G/fiber buildout) — FCF compressed
        elif capex_rev < 0.05 and stage in (CompanyStage.MATURE, CompanyStage.PLATFORM):
            score += 3  # asset-light comm-services platforms
...
def _score_growth(r: AnalysisReport) -> float:
    ...
    # R&D + content intensity — innovation/content commitment proxy
    invest = _safe(g.rd_intensity, None) or _safe(g.content_intensity, None)
    if invest is not None:
        if invest >= 0.15 and stage in (CompanyStage.STARTUP, CompanyStage.GROWTH, CompanyStage.SCALE):
            score += 5
        elif invest < 0.02 and stage in (CompanyStage.GROWTH, CompanyStage.SCALE):
            score -= 5  # under-investing during growth phase

    # Subscriber growth signal (when disclosed)
    sub_g = _safe(g.subscriber_growth_yoy, None)
    ...
    # Engagement signal (DAU/MAU)
    dau_mau = _safe(g.dau_mau_ratio, None)
...
def _tech_screening(r: AnalysisReport) -> dict:
    """Ten-point Communication Services screening checklist."""
    checks: dict = {}
    p, s, g, ss, tq = r.profitability, r.solvency, r.growth, r.share_structure, r.comm_quality
    stage = r.profile.stage

    # 1. Rule of 40 pass (≥40%) — primarily for digital subsegments
    ...
    # 2. Gross margin > 50% (sub-sector adjusted) — moat / network / content depth
    gm = _safe(p.gross_margin, None) if p else None
    checks["moat_gross_margin"] = gm > 0.50 if gm is not None else None

    # 3. SBC < 15% of revenue — discipline (less concerning for carriers/agencies)
    sbc = _safe(p.sbc_to_revenue, None) if p else None
    checks["sbc_contained"] = sbc < 0.15 if sbc is not None else None

    # 4. Share dilution <5%/yr (or net buybacks)
    dil = _safe(g.shares_growth_yoy, None) if g else None
    checks["low_dilution"] = dil < 0.05 if dil is not None else None

    # 5. FCF positive / cash runway adequate
    ...
    # 6. Net Debt / EBITDA < 4.0x — leverage discipline (carrier/cable critical)
    nde = _safe(s.net_debt_to_ebitda, None) if s else None
    checks["leverage_discipline"] = nde < 4.0 if nde is not None else None

    # 7. Capex discipline — capex/revenue not extreme (sub-sector adjusted)
    capex_rev = _safe(s.capex_to_revenue, None) if s else None
    if capex_rev is not None:
        checks["capex_discipline"] = capex_rev < 0.30
    else:
        checks["capex_discipline"] = None

    # 8. Insider ownership OR founder-led (common in internet/social/gaming)
    ...
    # 9. Revenue growth positive (any growth = winning vs sub-sector)
...
def _build_summaries(r: AnalysisReport) -> dict[str, str]:
    ...
    summaries["comm_quality"] = (r.comm_quality.competitive_position or "N/A") if r.comm_quality else "N/A"
    return summaries
...
def _tier_note(tier: CompanyTier) -> str:
    return {
        CompanyTier.MEGA: "Mega-cap: dominant carrier or platform. DCF, FCF yield, and durable-quality factors dominate.",
        CompanyTier.LARGE: "Large-cap: established operator. Balanced growth + FCF + leverage discipline.",
        CompanyTier.MID: "Mid-cap: watch ARPU, churn, and capex cadence carefully.",
        CompanyTier.SMALL: "Small-cap comm-services: execution & financing risk elevated. Runway, leverage, and content economics critical.",
        CompanyTier.MICRO: "Micro-cap: survival metrics dominate. Cash runway, leverage, SBC are must-checks.",
        CompanyTier.NANO: "Nano-cap: highly speculative. Most metrics unreliable — asset / spectrum / content backing only.",
    }.get(tier, "")


def _stage_note(stage: CompanyStage) -> str:
    return {
        CompanyStage.PLATFORM: "Dominant Platform: network effects / spectrum dominance / content library compound for decades. DCF & FCF yield primary.",
        CompanyStage.MATURE: "Mature / Cash-Generative: predictable FCF, buybacks, dividends. EV/EBITDA, EV/(EBITDA-Capex), P/FCF primary.",
        CompanyStage.SCALE: "Scale-Up: subscriber base expanding, profitability on the horizon. Watch ARPU/churn/CPGA trajectory.",
        CompanyStage.GROWTH: "Subscriber/User Growth: net adds, ARPU, engagement (DAU/MAU) are the quality gates.",
        CompanyStage.STARTUP: "Startup: product-market-fit risk dominates. Cash runway, engagement, and content/network-seed are key.",
    }.get(stage, "")
