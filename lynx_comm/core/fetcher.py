...
from lynx_comm.core.storage import get_financials_dir, save_json
from lynx_comm.models import CompanyProfile, FinancialStatement
...
def fetch_financial_statements(ticker: str) -> list[FinancialStatement]:
    ...
    if income is not None and not income.empty:
        for col in income.columns:
            ...
            # IT-specific income-statement items
            stmt.research_development = _get(income, col, "Research And Development", "Research Development")
            stmt.selling_general_admin = _get(income, col, "Selling General And Administration", "Selling General Administrative")
            ...
            if balance is not None and col in balance.columns:
                ...
                # IT-specific balance-sheet items
                stmt.deferred_revenue = _get(balance, col, "Current Deferred Revenue", "Deferred Revenue", "Non Current Deferred Revenue")
                stmt.goodwill = _get(balance, col, "Goodwill")
                stmt.intangibles = _get(balance, col, "Other Intangible Assets", "Intangible Assets Net Excluding Goodwill", "Net Tangible Assets")
            ...
            if cashflow is not None and col in cashflow.columns:
                ...
                # IT-specific cash-flow items
                stmt.stock_based_compensation = _get(cashflow, col, "Stock Based Compensation")
