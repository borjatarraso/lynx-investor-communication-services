...
from lynx_comm.models import AnalysisReport, CompanyStage, Severity
...
Sub-Sector: {esc(_ev(p.comm_category))} &nbsp;|&nbsp;
...
""")
    ...
    if report.valuation:
        ...
        parts.append(_metric_table([
            ...
            ("EV/Gross Profit", _fmt_ratio(v.ev_gross_profit)),
            ("EV/ARR (approx)", _fmt_ratio(v.ev_to_arr)),
            ("R40-Adj EV/Revenue", _fmt_ratio(v.rule_of_40_adj_multiple)),
            ("EV/Employee", _fmt_money(v.ev_per_employee)),
    ...
    if report.solvency:
        ...
        parts.append(_metric_table([
            ...
            ("Capex / Revenue", _fmt_pct(s.capex_to_revenue)),
            ("Deferred Rev / Revenue", _fmt_pct(s.deferred_revenue_ratio)),
            ("Goodwill / Assets", _fmt_pct(s.goodwill_to_assets)),
            ("Cash Coverage (months)", f"{s.cash_coverage_months:.0f}" if _safe(s.cash_coverage_months) is not None else "N/A"),
        ]))
        parts.append("</div>")
    ...
    if report.comm_quality:
        mq = report.comm_quality
        parts.append('<div class="card"><h2>Sector Quality Indicators</h2>')
        parts.append(_metric_table([
    ...
    if report.intrinsic_value:
        ...
        iv_fields = [
            ...
            _iv_entry("EV/Sales Implied", iv.ev_sales_implied_price, iv.margin_of_safety_ev_sales),
            ("Reverse DCF Growth", f"{iv.reverse_dcf_growth*100:.1f}%" if _safe(iv.reverse_dcf_growth) is not None else None),
        ]
        iv_fields = [f for f in iv_fields if f and f[1] is not None]
    ...
    if report.market_intelligence:
        ...
        if mi.benchmark_name or mi.sector_etf_name:
            parts.append("<h2>Comm-Services Benchmark &amp; Sector Context</h2>")
            ctx_fields = []
            if mi.benchmark_name and mi.benchmark_price:
                ctx_fields.append(("Benchmark", f"{mi.benchmark_name} — ${mi.benchmark_price:,.2f}"))
                if mi.benchmark_52w_high and mi.benchmark_52w_low:
                    ctx_fields.append(("52W Range", f"${mi.benchmark_52w_low:,.2f} — ${mi.benchmark_52w_high:,.2f}"))
    ...
    if c.screening_checklist:
        parts.append("<h2>Comm-Services Screening Checklist</h2>")
    ...
    parts.append(f"""
<div class="footer">
  Lynx Communication Services Analysis &mdash; Lince Investor Suite<br>
