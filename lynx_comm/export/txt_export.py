...
def export_txt(report: AnalysisReport, output_path: Path) -> Path:
    ...
    lines += _header(f"LYNX Communication Services  --  {p.name} ({p.ticker})")
    lines.append(f"  Tier: {_ev(p.tier)}  |  Stage: {_ev(p.stage)}")
    lines.append(f"  Sub-Sector: {_ev(p.comm_category)}  |  Jurisdiction: {_ev(p.jurisdiction_tier)}")
    ...
    if report.market_intelligence:
        ...
        if mi.benchmark_name or mi.sector_etf_name:
            lines.append("")
            lines.append("  Comm-Services Benchmark & Sector Context:")
    ...
    if c.screening_checklist:
        lines.append("")
        lines.append("  Comm-Services Screening Checklist:")
        lines.append("  " + "-" * 40)
    ...
    lines.append(f"  Lynx Communication Services (Lince Investor Suite)")
    lines.append("=" * W)
