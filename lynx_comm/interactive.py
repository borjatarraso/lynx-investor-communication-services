"""Interactive prompt-based mode for lynx-comm."""
...
from lynx_comm.core.news import download_article
from lynx_comm.core.reports import download_filing
from lynx_comm.display import display_full_report, display_report_stage
from lynx_comm.models import AnalysisReport

console = Console()

BANNER = "\n[bold blue]  L Y N X   Communication Services Analysis[/]\n[dim]    Telecom, Streaming, Internet, Social, Advertising, Publishing, Gaming, Broadcasting[/]\n"
...
def run_interactive():
    from lynx_comm.core.storage import get_mode, is_testing
    ...
    while True:
        ...
        try:
            console.print(f"\n[bold {prompt_color}]lynx-comm[/] ", end="")
            raw = input().strip()
        ...
        if cmd in ("quit", "exit", "q"):
            ...
            from lynx_comm import get_about_text; from rich.panel import Panel as P
            a = get_about_text()
            logo = a.get('logo_ascii', '')
            logo_block = f"[bold green]{logo}[/]\n" if logo else ""
            console.print(P(f"{logo_block}[bold blue]{a['name']} v{a['version']}[/]\n[dim]Part of {a['suite']} v{a['suite_version']}[/]\n\n[bold]By:[/] {a['author']}\n\n[dim]{a['description']}[/]",
            ...
            from lynx_comm.core.ticker import search_companies, display_search_results
            ...
            if current_report:
                from lynx_comm.display import _display_comm_quality, _display_share_structure, _display_intrinsic_value, _display_market_intelligence
                _display_comm_quality(current_report); _display_share_structure(current_report)
            ...
            if current_report and current_report.filings:
                from lynx_comm.display import _display_filings; _display_filings(current_report)
            else: console.print("[yellow]No filings available.[/]")
        elif cmd == "news":
            if current_report and current_report.news:
                from lynx_comm.display import _display_news; _display_news(current_report)
            ...
            from lynx_comm.core.storage import list_cached_tickers, get_mode
            ...
            from lynx_comm.core.storage import drop_cache_all, drop_cache_ticker, get_mode
            if arg.lower() == "all":
            ...
            from lynx_comm.metrics.explanations import get_section_explanation, SECTION_EXPLANATIONS
            if not arg:
            ...
            from lynx_comm.metrics.explanations import get_conclusion_explanation, CONCLUSION_METHODOLOGY
            ...
            from lynx_comm.easter import rich_matrix; rich_matrix(console)
        elif cmd == "fortune":
            from lynx_comm.easter import rich_fortune; rich_fortune(console)
        elif cmd == "rocket":
            from lynx_comm.easter import rich_rocket; rich_rocket(console)
        elif cmd == "lynx":
            from lynx_comm.easter import rich_lynx; rich_lynx(console)
        else:
